...
def get_rouge(pred_file, ref_file):
...
def get_topic_acc(pred_file, ref_file):
    count = 0
    refs = get_sents_str(ref_file)
    preds = get_sents_str(pred_file)
    for ref, pred in zip(refs, preds):
...
if __name__ == '__main__':
    pred_file = 'output/CSDS_bart_base_unsupervised_small//test_generations.txt'
    ref_file = 'CSDS_data/final_refs_test.txt'
    get_rouge(pred_file, ref_file)
