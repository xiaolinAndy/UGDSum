...
class WBDataset(Dataset):
    ...
    def __init__(self, data, tokenizer, dataset, max_history=15, batch_first=True, lm_labels=True):
        self.data = data
        self.tokenizer = tokenizer
        self.max_history = max_history
        self.pad = tokenizer.convert_tokens_to_ids(['<|endoftext|>'])[0]
    ...
    def process(self, history, resposne, with_eos=True):
        if self.dataset == 'samsum':
            bos, eos = self.tokenizer.convert_tokens_to_ids(['<|endoftext|>', '<|endoftext|>'])
        else:
        ...
        if self.dataset == 'CSDS':
            instance["token_type_ids"] = [bos]
            for i in range(len(history)):
                if self.tokenizer.convert_tokens_to_ids(['客'])[0] == history[i][0]:
                    instance["token_type_ids"] += [1] * len(history[i])
                else:
                    instance["token_type_ids"] += [0] * len(history[i])

            if self.tokenizer.convert_tokens_to_ids(['客'])[0] == resposne[0]:
